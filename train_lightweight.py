...
class EnhancedTransforms:
    ...
    def __call__(self, image, mask, confidence):
        ...
        if self.is_train:
            # Random brightness and contrast (increased probability and range)
            ...
            # Random rotation (small angles to preserve depth validity)
            if np.random.rand() > 0.5:
                ...
                confidence = cv2.warpAffine(confidence, M, (confidence.shape[1], confidence.shape[0]), 
                                          flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REFLECT)
            
            # Random crop and resize (simulating zoom)
            if np.random.rand() > 0.5:
                crop_factor = np.random.uniform(0.8, 1.0)
                crop_h = int(image.shape[0] * crop_factor)
                crop_w = int(image.shape[1] * crop_factor)
                ...
                conf_crop = confidence[start_h:start_h+crop_h, start_w:start_w+crop_w]
                
                image = cv2.resize(image_crop, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LINEAR)
                mask = cv2.resize(mask_crop, (mask.shape[1], mask.shape[0]), interpolation=cv2.INTER_NEAREST)
                confidence = cv2.resize(conf_crop, (confidence.shape[1], confidence.shape[0]), interpolation=cv2.INTER_NEAREST)
            
            # Implementation of CutDepth technique for depth estimation
            # if np.random.rand() > 0.7:
            #     h, w = mask.shape
            #     patch_size_h = np.random.randint(h // 8, h // 4)
            #     patch_size_w = np.random.randint(w // 8, w // 4)
                
            #     y = np.random.randint(0, h - patch_size_h)
            #     x = np.random.randint(0, w - patch_size_w)
                
            #     y2 = np.random.randint(0, h - patch_size_h)
            #     x2 = np.random.randint(0, w - patch_size_w)
                
            #     source_patch = mask[y2:y2+patch_size_h, x2:x2+patch_size_w]
            #     source_conf = confidence[y2:y2+patch_size_h, x2:x2+patch_size_w]
                
            #     if np.mean(source_conf) > 0.5:
            #         depth_offset = np.mean(mask[y:y+patch_size_h, x:x+patch_size_w]) - np.mean(source_patch)
            #         adjusted_patch = source_patch + depth_offset
                    
            #         # Create blend mask for smooth transitions
            #         blend_mask = np.ones((patch_size_h, patch_size_w), dtype=np.float32)
            #         blend_border = max(3, patch_size_h // 10)
                    
            #         for i in range(blend_border):
            #             alpha = i / blend_border
            #             blend_mask[i, :] *= alpha
            #             blend_mask[patch_size_h-i-1, :] *= alpha
            #             blend_mask[:, i] *= alpha
            #             blend_mask[:, patch_size_w-i-1] *= alpha
                    
            #         original = mask[y:y+patch_size_h, x:x+patch_size_w].copy()
            #         mask[y:y+patch_size_h, x:x+patch_size_w] = (
            #             original * (1 - blend_mask) + adjusted_patch * blend_mask
            #         )
...
def train_model(model, train_loader, val_loader, criterion, optimizer, 
    ...
    """
    Training function with mixed precision (FP16) using autocast
    """
    ...
    for epoch in range(num_epochs):
        ...
        for batch in progress_bar:
            ...
            # Update scheduler if using batch-wise scheduler
        ...
        # Update epoch-wise scheduler if not OneCycleLR
        if scheduler is not None and not isinstance(scheduler, torch.optim.lr_scheduler.OneCycleLR):
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                scheduler.step(metrics_avg['abs_rel'])
            else:
                scheduler.step()
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            visualize_predictions(model, val_loader, device, epoch+1, save_dir=f"{save_dir}/visualizations")
...
def main():
    ...
    # Hyperparameters - modified for better training
    batch_size = 128 # Reduced batch size for stability
    learning_rate = 5e-4
    num_epochs = 150
    input_size = (256, 320)  # Reduced input size for efficiency
    ...
    # Use the enhanced transforms
    train_transform = EnhancedTransforms(input_size=input_size, is_train=True)
    val_transform = EnhancedTransforms(input_size=input_size, is_train=False)
    ...
    # Create models
    model = LightweightDepthModel(pretrained=True, dropout_rate=0.2)
    model = model.to(device)
    for param in model.parameters():
        param.data = param.data.to(device)

    # Create a simpler loss function
    criterion = DepthLoss(alpha=0.85, beta=0.1, gamma=0.05)
    ...
    results_dir = "./lightweight_results"
    ...
    torch.save(quantized_model.state_dict(), f"{results_dir}/quantized_model.pth")
    print("Model quantized and saved for CPU deployment!")
